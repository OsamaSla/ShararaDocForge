...
function ManagementPanel({ company, onCompanyChange, showToast, onRefreshArchive }) {
  ...
  const [selectedSerialType, setSelectedSerialType] = useState(DOCUMENT_TYPES[0].value);
  ...
  const currentCounterVal = counters[selectedSerialType] || 0;
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                onChange={(e) => setSelectedSerialType(e.target.value)}
                className="form-input flex-1"
              >
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setSerial(selectedSerialType, num);
                }}
...
//  Dual-list management for customers & projects.
//  Projects are stored as a sub-collection of each
//  customer (Parent → Child dependency).
// ═══════════════════════════════════════════════
...
function DbManagementPanel() {
  ...
  const renderListItem = (name, type) => {
    const isSelected = type === "customer" ? selectedCustomer === name : selectedProject === name;
    const isEditing = type === "customer" ? editingCust === name : editingProj === name;
    const editVal = type === "customer" ? editCustVal : editProjVal;
    const setEditVal = type === "customer" ? setEditCustVal : setEditProjVal;
    const onSave = type === "customer" ? () => handleRenameCustomer(name) : () => handleRenameProject(name);
    const onCancel = () => {
      if (type === "customer") setEditingCust(null);
      else setEditingProj(null);
    };

    if (isEditing) {
      return (
        <div key={name} className="flex items-center gap-2 px-3 py-2 bg-blue-50 border-b border-blue-100 last:border-b-0">
          <input
            autoFocus
            value={editVal}
          ...
          />
          <button onClick={onSave} className="text-green-600 hover:text-green-800 text-xs font-semibold px-2 py-1 shrink-0">✓</button>
          <button onClick={onCancel} className="text-gray-400 hover:text-gray-600 text-xs px-2 py-1 shrink-0">✕</button>
        </div>
      );
    }
    ...
    return (
      <div
        ...
        className={`flex items-center justify-between px-3 py-2.5 cursor-pointer border-b border-gray-100 last:border-b-0 group transition-colors ${
          isSelected ? "bg-blue-50 hover:bg-blue-100" : "hover:bg-gray-50"
        }`}
      >
        <span className={`text-sm truncate ${isSelected ? "text-blue-700 font-medium" : "text-gray-700"}`}>{name}</span>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (type === "customer") handleDeleteCustomer(name);
              else handleDeleteProject(name);
            }}
  ...
  return (
    <div dir="rtl" className="w-full max-w-5xl mx-auto p-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* ── Customers Card ── */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex flex-col min-w-0">
          <h4 className="text-sm font-bold text-gray-700 mb-4">לקוחות ({customers.length})</h4>
          ...
          <div className="flex items-center gap-2 mb-4 w-full">
            <input
              ...
              className="flex-1 min-w-0 w-full border border-gray-300 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <button
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden max-h-[300px] overflow-y-auto bg-gray-50/30 w-full">
            {customers.length === 0 ? (
              <p className="text-gray-400 text-xs text-center py-8">אין לקוחות במאגר</p>
            ) : (
              customers.map((c) => renderListItem(c, "customer"))
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex flex-col min-w-0">
          <h4 className="text-sm font-bold text-gray-700 mb-4">
            פרויקטים {selectedCustomer ? `— ${selectedCustomer}` : ""}
          </h4>
          
          <div className="flex items-center gap-2 mb-4 w-full">
            <input
              type="text"
              placeholder={selectedCustomer ? "שם פרויקט חדש..." : "בחר לקוח תחילה כדי להוסיף פרויקט"}
              value={newProj}
              onChange={(e) => setNewProj(e.target.value)}
              disabled={!selectedCustomer}
              onKeyDown={(e) => { if (e.key === "Enter") handleAddProject(); }}
              className="flex-1 min-w-0 w-full border border-gray-300 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 disabled:placeholder-gray-400"
            />
            <button
              onClick={handleAddProject}
              disabled={!selectedCustomer}
              className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors shrink-0"
            >
              + הוסף
            </button>
          </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden max-h-[300px] overflow-y-auto bg-gray-50/30 w-full">
            {!selectedCustomer ? (
              <p className="text-gray-400 text-xs text-center py-8">בחר לקוח מהרשימה מימין כדי לנהל את הפרויקטים שלו</p>
            ) : projects.length === 0 ? (
              <p className="text-gray-400 text-xs text-center py-8">אין פרויקטים ללקוח זה</p>
            ) : (
